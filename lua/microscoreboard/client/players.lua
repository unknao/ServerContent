...
surface.CreateFont("MICRO_SCOREBOARDboard_16",{
	font = "Better VCR",
	size = 11,
	weight = 550
})
...
local function CreateMenuPanel(ply)
	...
	CopyMenu:SetMaterial(svg.Cache["MS_DMenu_Copy"].mat)

	CopySubMenu:AddOption("Name", function() SetClipboardText(ply:Name()) end):SetMaterial(svg.Cache["MS_DMenu_CopyName"].mat)
	CopySubMenu:AddOption("buttonProfile URL", function() SetClipboardText("http://steamcommunity.com/profiles/" .. ply:SteamID64()) end):SetMaterial(svg.Cache["MS_DMenu_CopybuttonProfileURL"].mat)
	CopySubMenu:AddOption("Model", function() SetClipboardText(ply:GetModel()) end):SetMaterial(svg.Cache["MS_DMenu_CopyModel"].mat)
	CopySubMenu:AddSpacer()
	CopySubMenu:AddOption("SteamID", function() SetClipboardText(ply:SteamID()) end):SetMaterial(svg.Cache["MS_DMenu_CopySteamID"].mat)
	CopySubMenu:AddOption("SteamID64", function() SetClipboardText(ply:SteamID64()) end):SetMaterial(svg.Cache["MS_DMenu_CopySteamID64"].mat)
	CopySubMenu:AddOption("AccountID", function() SetClipboardText(ply:AccountID()) end):SetMaterial(svg.Cache["MS_DMenu_CopyAccountID"].mat)
	CopySubMenu:AddOption("UserID", function() SetClipboardText(ply:EntIndex()) end):SetMaterial(svg.Cache["MS_DMenu_CopyUserID"].mat)
	...
	if ctrl and LocalPlayer() ~= ply then
		...
		if LocalPlayer():IsAdmin() then
			...
			ParentMenu:SetMaterial(svg.Cache["MS_DMenu_Goto"].mat)
			SubMenu:AddOption("Bring", function() ctrl.CallCommand(LocalPlayer(), "bring", {target}, target) end):SetMaterial(svg.Cache["MS_DMenu_Bring"].mat)

			Menu:AddOption("Kick", function() ctrl.CallCommand(LocalPlayer(), "kick", {target}, target) end):SetMaterial(svg.Cache["MS_DMenu_Kick"].mat)
		else
			Menu:AddOption("Go To", function() ctrl.CallCommand(LocalPlayer(), "goto", {target}, target) end):SetMaterial(svg.Cache["MS_DMenu_Goto"].mat)
		end
	end
	Menu:AddSpacer()
...
function PANEL:Paint(w, h)
	...
	--Text
	draw.SimpleText(ply:Name(), "MICRO_SCOREBOARDboard_16", 25 + self.RankPadding, 10, color_black, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)

	draw.SimpleText(ply:IsBot() and "BOT" or ply:Ping(), "MICRO_SCOREBOARDboard_16", w - 5, 10, MICRO_SCOREBOARD.Player_PingColor, TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER)
	...
	draw.SimpleText(playtime .. format, "MICRO_SCOREBOARDboard_16", w - 80, 10, color_black, TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER)
...
hook.Add("OnNW3ReceivedEntityValue", "MICRO_SCOREBOARDboard_flag_update", function(entindex, _, id, var)
	for _, pnl in ipairs(ms_player_panels) do
		if pnl:GetPlayerID() ~= entindex then return end
		if (id ~= "country_code" or id ~= "country") then return end
