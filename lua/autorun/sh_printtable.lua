local function _toMarkup(val, spacing, colors, parsed)
    ...
    elseif type(val) == "table" then
        if parsed[val] then
            return {{v = "<"..(tostring(val) or "nil")..">", i = spacing, c = colors.table or colors["default"]}}
        else
            parsed[val] = true
...
local function table_tohtml(tbl, colors_override)
    return MarkupToHTML(table_tomarkup(tbl, colors_override))
end